...
def cmd_pipeline(args):
    ...
    print("🚀 Running Trading Pipeline v2...")
...
def cmd_status(args):
    ...
    for fname in ['app.yaml', 'stocks.yaml', 'strategies.yaml', 'risk.yaml']:
        ...
        print(f"{icon} Config: {fname}")

def main():
    parser = argparse.ArgumentParser(description="Cuihua Quant System CLI v2")
    subparsers = parser.add_subparsers(dest='command', help='Available commands')
    p = subparsers.add_parser('sync'); p.add_argument('--pool', default='watchlist'); p.add_argument('--days', type=int, default=5)
    p = subparsers.add_parser('analyze'); p.add_argument('--pool', default='watchlist'); p.add_argument('--top', type=int, default=5)
    p = subparsers.add_parser('backtest'); p.add_argument('--pool', default='watchlist'); p.add_argument('--capital', type=float, default=100000); p.add_argument('--start-date', default='2025-01-01')
    p = subparsers.add_parser('pipeline'); p.add_argument('--execute', action='store_true'); p.add_argument('--ml', action='store_true')
    p = subparsers.add_parser('rebalance'); p.add_argument('--pool', default='watchlist'); p.add_argument('--capital', type=float, default=100000); p.add_argument('--method', default='equal', choices=['equal', 'risk_parity'])
    p = subparsers.add_parser('report'); p.add_argument('--date', default=None); p.add_argument('--weekly', action='store_true')
    subparsers.add_parser('status')
    args = parser.parse_args()
    cmds = {'sync': cmd_sync, 'analyze': cmd_analyze, 'backtest': cmd_backtest, 'pipeline': cmd_pipeline, 'rebalance': cmd_rebalance, 'report': cmd_report, 'status': cmd_status}
